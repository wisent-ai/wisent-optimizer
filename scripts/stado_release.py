...
def declared_version(root: Path) -> str:
    ...
    matches = re.findall(r'^\s*version\s*=\s*["\']([^"\']+)["\']', setup, re.MULTILINE)
    if len(matches) != 1:
        raise SystemExit("setup.py must contain exactly one literal version")
    return matches[0]
...
def quality() -> None:
    ...
    if declared_version(source_dir()) != version:
        raise SystemExit("WISENT_VERSION does not match setup.py")
